...
async function supaFetch(path, options = {}, token = null) {
  ...
  if (!res.ok) {
    ...
    const msg = err.message || err.error_description || `HTTP ${res.status}`;
    // Sessão expirada — mensagem amigável
    if (res.status === 401 || msg.toLowerCase().includes("jwt")) {
...
function saveSession(u) {
  try { sessionStorage.setItem("a3_session", JSON.stringify(u)); } catch(_) {}
}
function clearSession() {
  try { sessionStorage.removeItem("a3_session"); } catch(_) {}
}
function loadSession() {
  try { const s = sessionStorage.getItem("a3_session"); return s ? JSON.parse(s) : null; } catch(_) { return null; }
}
...
function Dashboard({user,items,movements,clients}){
  ...
  const sd=myItems.map(item=>({...item,current:getStock(item),avgPrice:calcAvgPrice(item,movements)}));
...
function ItemDetail({item,items,setItems,movements,clients,token,user,onClose}){
  ...
  const mv=(movements||[]).filter(m=>m.item_id===item.id&&m.client_id===item.client_id).slice(0,10).reverse();
...
// ── MOVEMENT PAGE ──────────────────────────────────────────────────────────────
function MovementPage({user,items,setItems,movements,setMovements,clients,token,type}){
  const [cid,setCid]=useState(user.clientId||clients[0]?.id||1);
  ...
  async function sub(){
    if(!iid||!qty||parseFloat(qty)<=0)return;
    setSaving(true);
    try{
      const body={item_id:parseInt(iid),client_id:parseInt(cid),type,qty:parseFloat(qty),price:price?parseFloat(price):null,date,user_name:user.name,nf:nf||null,obs:obs||null};
      await supaFetch("/rest/v1/movements",{method:"POST",headers:{"Prefer":"return=minimal"},body:JSON.stringify(body)},token);
      const [freshM,freshI]=await Promise.all([
        supaFetch("/rest/v1/movements?select=*&order=date.desc&limit=500",{},token),
        supaFetch("/rest/v1/items?select=*&order=name",{},token),
      ]);
      setMovements(freshM||[]);
      setItems(freshI||[]);
      setLastSaved({qty,price,nf,date,itemName:si?.name});
    ...
    <>
      ...
      <div style={S.content}>
        <div style={{maxWidth:560}}>
          ...
          {ok&&lastSaved&&<div style={{background:`${C.success}22`,border:`1px solid ${C.success}`,borderRadius:8,padding:12,marginBottom:14}}>
            <div style={{color:C.success,fontWeight:700,marginBottom:6}}>✓ Movimentação registrada!</div>
            <div style={{fontSize:12,color:C.textMid,display:"flex",flexWrap:"wrap",gap:12}}>
              <span><strong>Item:</strong> {lastSaved.itemName}</span>
              <span><strong>Qtd:</strong> {lastSaved.qty} un</span>
              <span><strong>Data:</strong> {lastSaved.date}</span>
              {lastSaved.price&&<span><strong>Preço:</strong> R$ {lastSaved.price}</span>}
              {lastSaved.nf&&<span><strong>NF:</strong> {lastSaved.nf}</span>}
            </div>
          </div>}
          <div style={S.section}>
            ...
            <div style={{padding:22,display:"flex",flexDirection:"column",gap:13}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:isE?"1fr 1fr 1fr":"1fr 1fr",gap:12}}>
                ...
                <div><label style={S.label}>Data</label><input style={S.input} type="date" value={date} onChange={e=>setDate(e.target.value)}/></div>
              </div>
              ...
              {si&&<div style={{background:C.surfaceHigh,borderRadius:8,padding:12,fontSize:13,display:"flex",gap:20}}><span><span style={{color:C.textDim}}>Atual: </span><strong>{getStock(si)} un</strong></span><span><span style={{color:C.textDim}}>Mín: </span>{si.stock_min}</span><span><span style={{color:C.textDim}}>Máx: </span>{si.stock_max}</span></div>}
...
// ── ADJUST PAGE ────────────────────────────────────────────────────────────────
function AdjustPage({user,items,setItems,movements,setMovements,clients,token}){
  const [cid,setCid]=useState(user.clientId||clients[0]?.id||1);
  const [iid,setIid]=useState("");
  ...
  const ci=items.filter(i=>i.client_id===parseInt(cid));
  const si=ci.find(i=>i.id===parseInt(iid));
  const can=iid&&qty&&parseFloat(qty)>0&&obs;
  ...
  async function sub(){
    ...
    setSaving(true);
    try{
      const body={item_id:parseInt(iid),client_id:parseInt(cid),type:"ajuste",qty:sq,date,user_name:user.name,obs};
      await supaFetch("/rest/v1/movements",{method:"POST",headers:{"Prefer":"return=minimal"},body:JSON.stringify(body)},token);
      const [freshM,freshI]=await Promise.all([
        supaFetch("/rest/v1/movements?select=*&order=date.desc&limit=500",{},token),
        supaFetch("/rest/v1/items?select=*&order=name",{},token),
      ...
      <div style={S.content}>
        <div style={{maxWidth:540}}>
          ...
          <div style={S.section}>
            ...
            <div style={{padding:22,display:"flex",flexDirection:"column",gap:13}}>
              {!user.clientId&&<div><label style={S.label}>Condomínio</label><select style={S.select} value={cid} onChange={e=>{setCid(e.target.value);setIid("");}}>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>}
              <div><label style={S.label}>Item</label><select style={S.select} value={iid} onChange={e=>setIid(e.target.value)}><option value="">Selecione...</option>{ci.map(i=>{const cur=getStock(i);return <option key={i.id} value={i.id}>{i.name} ({i.code}) — {cur} un{cur<0?" ⚠":""}</option>;})}</select></div>
              ...
              {si&&qty&&<div style={{background:C.surfaceHigh,borderRadius:8,padding:12,fontSize:13}}><span style={{color:C.textDim}}>Atual: </span><strong>{getStock(si)} un</strong><span style={{color:C.textDim}}> → após: </span><strong style={{color:C.yellow}}>{getStock(si)+(dir==="positivo"?parseFloat(qty||0):-parseFloat(qty||0))} un</strong></div>}
...
// ── HISTORY PAGE ───────────────────────────────────────────────────────────────
function HistoryPage({user,items,movements,setMovements,setItems,clients,token}){
  ...
  const enriched=(movements||[])
    .filter(m=>cid==="todos"||m.client_id===parseInt(cid))
    .filter(m=>tf==="todos"||m.type===tf)
    .filter(m=>!dateFrom||m.date>=dateFrom)
    .filter(m=>!dateTo||m.date<=dateTo)
    .filter(m=>!nfFilter||String(m.nf||"").toLowerCase().includes(nfFilter.toLowerCase()))
    .map(m=>{
      const item=items.find(i=>i.id===m.item_id&&i.client_id===m.client_id);
      const cl=clients.find(c=>c.id===m.client_id);
      return{...m,itemName:item?.name||"—",itemCode:item?.code||"—",category:item?.category||"—",clientName:cl?.name||"—"};
    })
    .filter(m=>cf==="Todos"||m.category===cf)
    .filter(m=>{
      if(!search)return true;
      const q=search.toLowerCase();
      return [m.itemName,m.itemCode,m.category,m.clientName,m.nf,m.user_name,m.obs].some(v=>String(v||"").toLowerCase().includes(q));
    })
  ...
  async function saveEdit(){
    ...
    setSaving(true);
    try{
      await supaFetch(`/rest/v1/movements?id=eq.${editM.id}`,{
        method:"PATCH",
        headers:{"Prefer":"return=minimal"},
        body:JSON.stringify({
          qty:parseFloat(editM.qty),
          price:editM.price?parseFloat(editM.price):null,
          date:editM.date,
          nf:editM.nf||null,
          obs:editM.obs||null,
        }),
  ...
  async function confirmDelete(){
    ...
    setSaving(true);
    try{
      await supaFetch(`/rest/v1/movements?id=eq.${delM.id}`,{method:"DELETE",headers:{"Prefer":"return=minimal"}},token);
      await refresh();
      setDelM(null);
    }catch(e){alert(e.message);}
    finally{setSaving(false);}
  ...
  return(
    <>
      <div style={S.topbar}>
        <div><div style={{fontSize:18,fontWeight:700}}>◷ Histórico</div><div style={{fontSize:12,color:C.textMid,marginTop:2}}>{enriched.length} registros</div></div>
      </div>
      <div style={S.content}>
        {/* Filtros */}
        <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:12,color:C.textMid}}>De</span>
            <input style={{...S.input,width:130,margin:0}} type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/>
            <span style={{fontSize:12,color:C.textMid}}>até</span>
            <input style={{...S.input,width:130,margin:0}} type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)}/>
          </div>
          <input style={{...S.input,width:160,margin:0}} placeholder="Buscar NF..." value={nfFilter} onChange={e=>setNfFilter(e.target.value)}/>
          <input style={{...S.input,width:220,margin:0}} placeholder="🔍 Buscar produto, NF, responsável..." value={search} onChange={e=>setSearch(e.target.value)}/>
          {(dateFrom||dateTo||nfFilter||search)&&<button style={{...S.btnYellow,padding:"6px 12px",fontSize:12}} onClick={()=>{setDateFrom("");setDateTo("");setNfFilter("");setSearch("");}}>✕ Limpar</button>}
        </div>
        ...
        <div style={S.section}>
          <table style={S.table}>
            <thead><tr>
              ...
              <th style={S.th}>Qtd</th><th style={S.th}>Preço Un.</th><th style={S.th}>NF</th>
              <th style={S.th}>Obs</th><th style={S.th}>Responsável</th>
              {true&&<th style={S.th}></th>}
            </tr></thead>
            <tbody>
              {enriched.map(m=>(
                <tr key={m.id}>
                  ...
                  <td style={{...S.td,fontSize:12,color:C.textMid,maxWidth:160}}>{m.obs||"—"}</td>
                  <td style={{...S.td,fontSize:12}}>{m.user_name}</td>
                  {true&&(
                    <td style={S.td}>
                      <div style={{display:"flex",gap:6}}>
                        <button onClick={()=>setEditM({...m})} style={{background:C.surfaceHigh,border:`1px solid ${C.border}`,borderRadius:6,padding:"4px 10px",color:C.textMid,cursor:"pointer",fontSize:12}}>✏</button>
                        <button onClick={()=>setDelM(m)} style={{background:`${C.danger}18`,border:`1px solid ${C.danger}44`,borderRadius:6,padding:"4px 10px",color:C.danger,cursor:"pointer",fontSize:12}}>✕</button>
                      </div>
                    </td>
                  )}
                </tr>
              ))}
              {enriched.length===0&&<tr><td colSpan={11} style={{...S.td,textAlign:"center",color:C.textDim,padding:32}}>Nenhuma movimentação encontrada</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {editM&&(
        <div style={{position:"fixed",inset:0,background:"#000a",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
          <div style={{background:C.surface,borderRadius:12,padding:28,width:440,boxShadow:"0 8px 40px #0008"}}>
            <div style={{fontSize:16,fontWeight:700,marginBottom:20}}>✏ Editar Lançamento</div>
            <div style={{display:"flex",flexDirection:"column",gap:13}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div><label style={S.label}>Data</label><input style={S.input} type="date" value={editM.date} onChange={e=>setEditM({...editM,date:e.target.value})}/></div>
                <div><label style={S.label}>Quantidade</label><input style={S.input} type="number" value={editM.qty} onChange={e=>setEditM({...editM,qty:e.target.value})}/></div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div><label style={S.label}>Preço Unitário (R$)</label><input style={S.input} type="number" placeholder="—" value={editM.price||""} onChange={e=>setEditM({...editM,price:e.target.value})}/></div>
                <div><label style={S.label}>Nota Fiscal</label><input style={S.input} placeholder="—" value={editM.nf||""} onChange={e=>setEditM({...editM,nf:e.target.value})}/></div>
              </div>
              <div><label style={S.label}>Observação</label><input style={S.input} placeholder="—" value={editM.obs||""} onChange={e=>setEditM({...editM,obs:e.target.value})}/></div>
            </div>
            <div style={{display:"flex",gap:10,marginTop:20,justifyContent:"flex-end"}}>
              <button style={{...S.btnYellow,background:C.surfaceHigh,color:C.textMid}} onClick={()=>setEditM(null)}>Cancelar</button>
              <button style={{...S.btnYellow,opacity:saving?0.7:1}} onClick={saveEdit} disabled={saving}>{saving?"Salvando...":"Salvar"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {delM&&(
        <div style={{position:"fixed",inset:0,background:"#000a",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
          <div style={{background:C.surface,borderRadius:12,padding:28,width:380,boxShadow:"0 8px 40px #0008"}}>
            <div style={{fontSize:16,fontWeight:700,marginBottom:10,color:C.danger}}>⚠ Excluir Lançamento</div>
            <div style={{fontSize:13,color:C.textMid,marginBottom:20,lineHeight:1.6}}>
              Isso irá <strong style={{color:C.danger}}>remover permanentemente</strong> o lançamento de <strong>{delM.itemName}</strong> ({tl[delM.type]}, {delM.qty} un, {delM.date}) e ajustar o estoque automaticamente.
            </div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <button style={{...S.btnYellow,background:C.surfaceHigh,color:C.textMid}} onClick={()=>setDelM(null)}>Cancelar</button>
              <button style={{...S.btnYellow,background:C.danger,color:"#fff",opacity:saving?0.7:1}} onClick={confirmDelete} disabled={saving}>{saving?"Excluindo...":"Confirmar Exclusão"}</button>
            </div>
          </div>
        </div>
      )}
...
function ReportPage({items,movements,clients}){
  ...
  const cons=clients.map(cl=>{
    ...
    const tv=ci.reduce((s,item)=>s+Math.max(0,getStock(item))*calcAvgPrice(item,movements),0);
    ...
    const al=ci.filter(i=>getStock(i)<=i.stock_min).length;
    ...
    <>
      ...
      <div style={S.content}>
        <div style={S.section}>
          ...
          <tbody>{clients.map(c=>{const ci=items.filter(i=>i.client_id===c.id);const al=ci.filter(i=>getStock(i)<=i.stock_min).length;return(<tr key={c.id} style={{cursor:"pointer"}} onClick={()=>setSel(c)}><td style={S.td}><strong>{c.name}</strong></td><td style={{...S.td,fontSize:12,color:C.textMid}}>{c.cnpj}</td><td style={{...S.td,fontSize:12,color:C.textMid,maxWidth:180}}>{c.address}</td><td style={S.td}>{c.units}</td><td style={S.td}>{ci.length}</td><td style={S.td}>{al>0?<span style={bdg(C.danger)}>{al}</span>:<span style={bdg(C.success)}>0</span>}</td></tr>);})}</tbody></table>
        </div>
      </div>
...
function RequisitionsPage({user,items,movements,setMovements,clients,token,requisitions,setRequisitions,reqItems,setReqItems}){
  ...
  const pending=myReqs.filter(r=>r.status==="enviada").length;

  async function deleteReq(){
    if(!delReq)return;
    setSaving(true);
    try{
      await supaFetch(`/rest/v1/requisition_items?requisition_id=eq.${delReq.id}`,{method:"DELETE",headers:{"Prefer":"return=minimal"}},token);
      await supaFetch(`/rest/v1/requisitions?id=eq.${delReq.id}`,{method:"DELETE",headers:{"Prefer":"return=minimal"}},token);
      setRequisitions(requisitions.filter(r=>r.id!==delReq.id));
      setReqItems(reqItems.filter(i=>i.requisition_id!==delReq.id));
      setDelReq(null);
    }catch(e){alert(e.message);}
    finally{setSaving(false);}
  }
  ...
  async function saveEdit(){
    ...
    setSaving(true);
    try{
      await supaFetch(`/rest/v1/requisitions?id=eq.${editReq.id}`,{method:"PATCH",headers:{"Prefer":"return=minimal"},body:JSON.stringify({notes:editNotes||null})},token);
      setRequisitions(requisitions.map(r=>r.id===editReq.id?{...r,notes:editNotes||null}:r));
      setEditReq(null);
    }catch(e){alert(e.message);}
    finally{setSaving(false);}
  }
  ...
  return(
    <>
                  ...
                  <tr key={req.id} style={{cursor:"pointer"}} onClick={()=>setSel(req)}>
                    ...
                    <td style={S.td} onClick={e=>e.stopPropagation()}>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                        ...
                        {req.status==="aberta"&&<button style={{...S.btnSmall,color:C.textMid}} onClick={e=>{e.stopPropagation();setEditNotes(req.notes||"");setEditReq(req);}}>✏</button>}
                        {req.status==="aberta"&&<button style={{...S.btnSmall,color:C.danger}} onClick={e=>{e.stopPropagation();setDelReq(req);}}>✕</button>}
      ...
      {/* Modal Edição */}
      {editReq&&(
        <div style={S.overlay} onClick={()=>setEditReq(null)}>
          <div style={{...S.modal,maxWidth:440}} onClick={e=>e.stopPropagation()}>
            <div style={S.modalHead}>
              <div style={{fontWeight:700,fontSize:16}}>✏ Editar REQ-{String(editReq.id).padStart(3,"0")}</div>
              <button style={S.btnSmall} onClick={()=>setEditReq(null)}>✕</button>
            </div>
            <div style={S.modalBody}>
              <label style={S.label}>Observações</label>
              <textarea style={{...S.input,height:100,resize:"vertical"}} placeholder="Observações da requisição..." value={editNotes} onChange={e=>setEditNotes(e.target.value)}/>
            </div>
            <div style={S.modalFoot}>
              <button style={S.btnSecondary} onClick={()=>setEditReq(null)}>Cancelar</button>
              <button style={{...S.btnYellow,opacity:saving?0.7:1}} onClick={saveEdit} disabled={saving}>{saving?"Salvando...":"Salvar"}</button>
            </div>
          </div>
        </div>
      ...
      {/* Modal Exclusão */}
      {delReq&&(
        <div style={S.overlay} onClick={()=>setDelReq(null)}>
          <div style={{...S.modal,maxWidth:400}} onClick={e=>e.stopPropagation()}>
            <div style={S.modalHead}>
              <div style={{fontWeight:700,fontSize:16,color:C.danger}}>⚠ Excluir Requisição</div>
              <button style={S.btnSmall} onClick={()=>setDelReq(null)}>✕</button>
            </div>
            <div style={S.modalBody}>
              <div style={{fontSize:13,color:C.textMid,lineHeight:1.6}}>
                Deseja excluir permanentemente a <strong>REQ-{String(delReq.id).padStart(3,"0")}</strong>?<br/>
                Esta ação não pode ser desfeita.
              </div>
            </div>
            <div style={S.modalFoot}>
              <button style={S.btnSecondary} onClick={()=>setDelReq(null)}>Cancelar</button>
              <button style={{...S.btnYellow,background:C.danger,color:"#fff",opacity:saving?0.7:1}} onClick={deleteReq} disabled={saving}>{saving?"Excluindo...":"Confirmar Exclusão"}</button>
            </div>
          </div>
        </div>
      )}
...
export default function App(){
  ...
  // Restaurar sessão ao montar — tenta refresh imediato antes de carregar dados
  useEffect(()=>{
    const saved=loadSession();
    if(!saved?.token)return;
    (async()=>{
      try{
        // Sempre tenta renovar o token ao restaurar sessão
        const data=await doRefreshToken(saved.refreshToken);
        const updated={...saved,token:data.access_token,refreshToken:data.refresh_token};
        setUser(updated);
        saveSession(updated);
        loadData(updated.token);
      }catch(e){
        // Refresh falhou — tenta com token atual, se falhar faz logout
        try{
          await loadData(saved.token);
          setUser(saved);
        }catch(_){
          clearSession();
        }
  ...
  useEffect(()=>{
    ...
    const interval=setInterval(async()=>{
      try{
        const data=await doRefreshToken(user.refreshToken);
        ...
        setItems([]);setMovements([]);setClients([]);
      }
  ...
  function handleLogout(){
    clearSession();
    setUser(null);setPage("dashboard");
    setItems([]);setMovements([]);setClients([]);setSuppliers([]);
    setRequisitions([]);setReqItems([]);
  }
  ...
  const pages={
    ...
    entry:<MovementPage {...sharedProps} items={items} setItems={setItems} movements={movements} setMovements={setMovements} type="entrada"/>,
    exit:<MovementPage {...sharedProps} items={items} setItems={setItems} movements={movements} setMovements={setMovements} type="saida"/>,
    adjust:<AdjustPage {...sharedProps} items={items} setItems={setItems} movements={movements} setMovements={setMovements}/>,
    history:<HistoryPage {...sharedProps} items={items} setItems={setItems} movements={movements} setMovements={setMovements}/>,
  ...
  return(
    <div style={S.app}>
      <Shell user={user} page={page} setPage={setPage} clients={clients} requisitions={requisitions} onLogout={handleLogout}>
