...
function RequisitionsPage({user,items,movements,setMovements,clients,token,requisitions,setRequisitions,reqItems,setReqItems}){
  ...
  const statusColor={rascunho:C.textDim,enviada:C.blue,separada:C.warning,concluida:C.success,cancelada:C.danger};
  const statusLabel={rascunho:"Rascunho",enviada:"Enviada",separada:"Separada",concluida:"Concluída",cancelada:"Cancelada"};
                ...
                return(
                  <tr key={req.id} style={{cursor:"pointer"}} onClick={()=>setSel(req)}>
                    ...
                    <td style={S.td} onClick={e=>e.stopPropagation()}>
                      {req.status==="rascunho"&&<button style={{...S.btnSmall,color:C.blue}} onClick={e=>{e.stopPropagation();handleSendLink(req);}}>Enviar link</button>}
                      {req.status==="enviada"&&user.role!=="zelador"&&<button style={{...S.btnSmall,color:C.warning}} onClick={e=>{e.stopPropagation();setSel(req);}}>Separar</button>}
                      {req.status==="separada"&&<button style={{...S.btnSmall,color:C.success}} onClick={e=>{e.stopPropagation();window.open(`${window.location.origin}/req/${req.token}`,"_blank");}}>Link assinatura</button>}
                    </td>
                  </tr>
                );
              })}
...
function NewRequisitionModal({user,items,clients,token,requisitions,setRequisitions,reqItems,setReqItems,onClose}){
  const [cid,setCid]=useState(user.clientId||clients[0]?.id||1);
  const [lines,setLines]=useState([{item_id:"",qty:1}]);
  const [notes,setNotes]=useState("");
  const [saving,setSaving]=useState(false);
  const [err,setErr]=useState("");
  const clientItems=items.filter(i=>i.client_id===parseInt(cid));

  function addLine(){setLines([...lines,{item_id:"",qty:1}]);}
  function removeLine(i){setLines(lines.filter((_,j)=>j!==i));}
  function updateLine(i,field,val){setLines(lines.map((l,j)=>j===i?{...l,[field]:val}:l));}

  async function save(){
    const valid=lines.filter(l=>l.item_id&&l.qty>0);
    if(valid.length===0){setErr("Adicione ao menos um item.");return;}
    setSaving(true);
    try{
      const reqBody={client_id:parseInt(cid),created_by:user.name,notes:notes||null,status:"rascunho"};
      ...
      const itemsBody=valid.map(l=>({requisition_id:newReq.id,item_id:parseInt(l.item_id),client_id:parseInt(cid),qty_requested:parseFloat(l.qty)}));
      const resItems=await supaFetch("/rest/v1/requisition_items",{method:"POST",headers:{"Prefer":"return=representation"},body:JSON.stringify(itemsBody)},token);
      setRequisitions([newReq,...requisitions]);
      setReqItems([...reqItems,...(Array.isArray(resItems)?resItems:[resItems])]);
      onClose();
    }catch(e){setErr(e.message);}
    finally{setSaving(false);}
  }
  ...
  return(
    <div style={S.overlay} onClick={onClose}>
      <div style={{...S.modal,maxWidth:580}} onClick={e=>e.stopPropagation()}>
        <div style={S.modalHead}><div><div style={{fontWeight:700,fontSize:16}}>Nova Requisição</div><div style={{fontSize:12,color:C.textDim}}>Crie e envie o link para o colaborador</div></div><button style={S.btnSmall} onClick={onClose}>✕</button></div>
        <div style={S.modalBody}>
          {err&&<div style={{background:`${C.danger}22`,border:`1px solid ${C.danger}`,borderRadius:8,padding:10,marginBottom:12,fontSize:13,color:C.danger}}>{err}</div>}
          <div style={{display:"flex",flexDirection:"column",gap:13}}>
            {!user.clientId&&<div><label style={S.label}>Condomínio</label><select style={S.select} value={cid} onChange={e=>{setCid(e.target.value);setLines([{item_id:"",qty:1}]);}}>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>}
            <div>
              <label style={S.label}>Itens da Requisição</label>
              {lines.map((line,i)=>(
                <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 100px 36px",gap:8,marginBottom:8}}>
                  <select style={S.select} value={line.item_id} onChange={e=>updateLine(i,"item_id",e.target.value)}>
                    ...
                    {clientItems.map(it=><option key={it.id} value={it.id}>{it.name} ({it.code}) — {calcStock(it,[])} un</option>)}
                  </select>
                  <input style={S.input} type="number" min="1" placeholder="Qtd" value={line.qty} onChange={e=>updateLine(i,"qty",e.target.value)}/>
                  <button style={{...S.btnSmall,color:C.danger,padding:"5px 8px"}} onClick={()=>removeLine(i)}>✕</button>
                </div>
              ))}
              <button style={{...S.btnSmall,marginTop:4}} onClick={addLine}>+ Adicionar item</button>
            </div>
            <div><label style={S.label}>Observações</label><input style={S.input} placeholder="Opcional..." value={notes} onChange={e=>setNotes(e.target.value)}/></div>
          </div>
        </div>
        <div style={S.modalFoot}><button style={S.btnSecondary} onClick={onClose}>Cancelar</button><button style={{...S.btnYellow,opacity:saving?0.7:1}} onClick={save} disabled={saving}>{saving?"Salvando...":"Criar Requisição"}</button></div>
      </div>
    </div>
  );
}
...
function RequisitionDetail({req,reqItems,items,movements,setMovements,clients,token,user,requisitions,setRequisitions,setReqItems,onClose}){
  ...
  async function sendLink(){
    await supaFetch(`/rest/v1/requisitions?id=eq.${req.id}`,{method:"PATCH",body:JSON.stringify({status:"enviada",updated_at:new Date().toISOString()})},token);
    setRequisitions(requisitions.map(r=>r.id===req.id?{...r,status:"enviada"}:r));
    navigator.clipboard.writeText(link).then(()=>alert(`Status atualizado e link copiado!\n\n${link}`)).catch(()=>alert(`Link:\n\n${link}`));
  }
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={{...S.modal,maxWidth:620}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={S.modalFoot}>
          ...
          {req.status==="rascunho"&&<button style={S.btnPrimary} onClick={sendLink}>Enviar link ao colaborador</button>}
          {req.status==="enviada"&&user.role!=="zelador"&&<button style={{...S.btnYellow,opacity:saving?0.7:1}} onClick={confirmSeparation} disabled={saving}>{saving?"Salvando...":"Confirmar Separação"}</button>}
        </div>
      </div>
    </div>
  );
  ...
  useEffect(()=>{
    async function load(){
      try{
        ...
        setReqItems(ris||[]);
        const itemIds=[...new Set((ris||[]).map(i=>i.item_id))].join(",");
        if(itemIds){
          const its=await supaFetch(`/rest/v1/items?id=in.(${itemIds})&select=*`);
          setItems(its||[]);
        }
  ...
  async function confirm(){
    ...
    try{
      ...
      for(const ri of reqItems){
        ...
        const hasChange=qtyConf!==(ri.qty_separated??ri.qty_requested);
  ...
  if(step==="done")return(
    <div style={{...S.app,display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      <div style={{textAlign:"center",padding:32}}>
        ...
        <div style={{fontSize:22,fontWeight:700,color:C.success,marginBottom:8}}>Recebimento confirmado!</div>
        <div style={{fontSize:14,color:C.textMid}}>REQ-{String(req.id).padStart(3,"0")} · {collabName}</div>
      </div>
    </div>
  );

  const isForm=req.status==="enviada"||req.status==="separada";
        ...
        {isForm&&(
          <>
            <div style={{...S.section,marginBottom:16}}>
              <div style={S.sectionHead}><div style={{fontSize:14,fontWeight:700}}>Itens {isSep?"separados":"solicitados"}</div></div>
              <table style={S.table}>
                <thead><tr>
                  <th style={S.th}>Item</th>
                  <th style={S.th}>{isSep?"Separado":"Solicitado"}</th>
                  {isSep&&<th style={S.th}>Confirmar</th>}
                </tr></thead>
                <tbody>
                  {reqItems.map(ri=>{
                    const it=items.find(i=>i.id===ri.item_id);
                    const baseQty=isSep?ri.qty_separated:ri.qty_requested;
            ...
            {!isSep&&(
              <div style={{background:`${C.blue}18`,border:`1px solid ${C.blue}44`,borderRadius:10,padding:16,textAlign:"center"}}>
                <div style={{color:C.blue,fontWeight:600,marginBottom:4}}>Requisição recebida!</div>
                <div style={{fontSize:13,color:C.textMid}}>Aguarde a separação. Você receberá um novo link para confirmar o recebimento.</div>
              </div>
            )}
          </>
        )}
